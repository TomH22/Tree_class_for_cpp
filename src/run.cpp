#include "Staff.h"// includes Tree.h
...
int run() {
  ...
  bool bOrderAlphabetical = false;
  bool bOrderAlphabeticalSet = false;
  bool bShowStaffSergants = true;
  bool bShowSergants = true;
  bool bShowCorporals = true;
  bool bShowPrivates = true;
  ...
  while (!glfwWindowShouldClose(window))
  {
      ...
      if (bOrderAlphabetical)
      {
          treeModel.SortAllColumns();
          bOrderAlphabeticalSet = true;
      }

      // Tree
      {
          ImGui::Begin("All Staff");
          outStaff(treeModel.GetTreeItem());
          ImGui::NewLine();
          ImGui::Separator();
          if (bOrderAlphabeticalSet)
              bOrderAlphabetical = true;
          ImGui::Checkbox("Order alphabetical", &bOrderAlphabetical);
          ImGui::End();
      }

      // Settings
      {
          ImGui::Begin("Create list");
          ImGui::Checkbox("Show staff sergants", &bShowStaffSergants);
          ImGui::Checkbox("Show sergants", &bShowSergants);
          ImGui::Checkbox("Show corporals", &bShowCorporals);
          ImGui::Checkbox("Show privates", &bShowPrivates);
          ImGui::NewLine();
          ImGui::Separator();
          if (ImGui::Button(bShowList ? "Hide" : "Show"))
              bShowList = !bShowList;
          ImGui::End();
      }
      ...
      {
          ImGui::Begin("List");
          auto i = std::make_shared<StaffTopDownIterator>(treeModel, bOrderAlphabetical, bShowStaffSergants, bShowSergants, bShowCorporals, bShowPrivates);

          for (i->First(); !i->IsDone(); i->Next())
          {
              ImGui::Text(i->CurrentItem().GetNameAsSpChar().get());
          }
          ImGui::End();
      }
